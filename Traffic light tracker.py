...
def process_frame(frame):
    lower_range = np.array([79, 211, 85])  # Green color range
    upper_range = np.array([149, 255, 255])
    lower_range1 = np.array([47, 138, 183])  # Red color range
    upper_range1 = np.array([177,205, 255])
    lower_range2 = np.array([0, 173, 204])  # Red color range
    upper_range2 = np.array([119, 255, 255])
    ...
    combined_mask = cv2.bitwise_or(mask, mask1,mask2)
        ...
        if cv2.contourArea(c) > 50:
            ...
            # Determine the color of the contour
                if cv2.countNonZero(mask[y:y+h, x:x+w]) > 0:  # Green color range
                   ...
                   label = "RED"
                elif cv2.countNonZero(mask1[y:y + h, x:x + w]) > 0:  # Yellow  color range
                    color = (0, 255, 255)  # Yellow color for the rectangle
